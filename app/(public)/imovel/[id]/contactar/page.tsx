...
export default async function ContactarPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = await params;
    const user = await getSessionUser();

    if (!user) {
        redirect(`/login?redirect=/imovel/${id}/contactar`);
    }
            ...
            <div className="container-page">
                <div className="mx-auto max-w-2xl">
                    ...
                    <div className="mt-6 card overflow-hidden">
                        ...
                        <div className="p-6">
                            <h2 className="text-xl font-bold text-slate-900">Contactar anunciante</h2>
                            <p className="mt-1 text-sm text-slate-600">
                                Envie uma mensagem a {property.owner.name} sobre este imóvel.
                            </p>

                            <div className="mt-6">
                                <ContactForm propertyId={id} propertyTitle={property.title} />
                            </div>
                        </div>
                    </div>
                </div>
